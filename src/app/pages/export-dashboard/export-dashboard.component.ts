...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, MatCardModule, MatSelectModule, FormsModule, MatIcon, ReactiveFormsModule, ShowDialogComponent],
...
export class ExportDashboardComponent implements AfterViewInit {
    ...
    exportForm: FormGroup = new FormGroup({
        ...
        currency: new FormControl(this.selectCurrencyCode, [Validators.required, Validators.maxLength(3), Validators.pattern(/^[A-Za-z]+$/)]),
    });
    ...
    focusField(controlName: string) {
        const input: HTMLInputElement | null = this.el.nativeElement.querySelector(`input[formControlName="${controlName}"]`) as HTMLInputElement;
        if (input) {
            input.focus();
        }
    }
    ...
    loadDashboardData(CurrYear: string, Usercurrency: string): void {
        this.exportDashboardService.fetchTotalExportSummary("/exportDashboard/expHistory", { year: CurrYear.toString(), currency: Usercurrency.toString() }).subscribe({
            next: (data) => {
